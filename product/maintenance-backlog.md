...
| M-37 | **The guard had a rename hole and a typechange hole.** `--diff-filter=MDR` with default rename detection prints only the **destination** path, so `git mv .../expected.json .../expected.json.bak` in an engine PR matched no rule and the guard **PASSED** while the fixture was effectively gone. `T` (replacing a fixture with a symlink) was in none of M/D/R | guard hole | **FIXED** — `--no-renames --diff-filter=MDT`. Verified by negative control: the old form saw *nothing* on the simulated rename; the new form catches both. Found by Code Review |
