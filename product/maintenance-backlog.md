...
| M-36 | The fixture-immutability guard's `git fetch --no-tags --depth=0 ... \|\| true` was a **no-op**: `--depth=0` is not a meaningful depth argument, so the fallback that exists to make the guard robust did nothing | dead fallback | **FIXED** in this PR (`--unshallow`, with a plain fetch behind it). Caught by Code Review. Harmless only because `fetch-depth: 0` on the checkout already supplied full history — i.e. the guard worked for a reason unrelated to the line intended to make it work |
