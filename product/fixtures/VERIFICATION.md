...
> not something this branch built.
>
...
Project Audit and Strategic Review. The chain ran against **every head of this audit**, beginning `c27a2d6` → `c0ede4d` →
`9d704e0` → `c10612c` → `53b554a` → `4532492` → `a1e6a0f` and continuing to the exact final
head of [PR #18](https://github.com/tomerYannay/4UR4/pull/18). *(A literal head or round
count is deliberately not asserted anywhere in this file: each correction commit would
falsify it, which is the exact defect class this table exists to record.)* This table records what
each round found and how it was corrected; a cell annotated "corrected again" was fixed in a
...
| **The decision-register provenance for HD-12/13/14 was overstated**, and HD-13 cited an artifact that did not exist. | Corrected in `human-decisions.md`: all three disclosed that the ruling had reached the repository as a direct Product Owner instruction to the session, with no posted GitHub artifact, and that ratification was outstanding. **Since closed** — the Product Owner ratified on 2026-07-25 ([artifact](https://github.com/tomerYannay/4UR4/issues/16#issuecomment-5080542012), against head `2651cd0`), and the provenance blocks are retained as history with the gap marked closed. |
