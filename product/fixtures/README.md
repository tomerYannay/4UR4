...
figures**. *Stated precisely: three of the five — the Phase 2 planner, the
...
are not an artifact of a single model.*
...
geometry. `tools/fixture-replay.mjs` **did not replay RM-01 at all** — it derived its
fixture list from `golden/` and never read `real/`. The real-market layer, which §6 names
as the **only** non-circular evidence in the corpus, was therefore always checked
*less* than the synthetic layer it exists to keep honest. **This is now closed:** RM-01
carries [`real/RM-01/expected-causal.json`](real/RM-01/expected-causal.json) and is
asserted by `--all` in CI — though as a **replay-generated regression guard**, which
closes the *absence of a guard*, not the absence of independent verification. That is why
this survived the
...
The scope limits travel with that answer, and this file states them because it is where
the evidence lives. **They are numbered 1, 1b, 2, 3, 4 to match
[`../human-decisions.md`](../human-decisions.md), which is authoritative** — this list
previously renumbered them 1–4, which made "limit 3" and "limit 4" name different things
in different files:
...
- **1b. The stop index must be engine-derived, not fixture-supplied.** If the harness may
  hand the engine the bar-10 stop, the B-clause asserts nothing about the engine's own
  detection — which is the whole point of gating it.
2. **Half B *narrows* RM-01's Phase-2 assertable surface.** Under Half A alone
...
3. **Circularity limit — the condition HOLDS; this is no longer conditional.** The Half B
   expectation **is** generated by
   `../../tools/fixture-replay.mjs`. It **is model-derived**, and is therefore a **regression
